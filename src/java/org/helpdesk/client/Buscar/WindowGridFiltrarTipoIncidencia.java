 ...
 */
public class WindowGridFiltrarTipoIncidencia extends ConfiguracionUI{
        ...
        private Grid<BaseModelIncidencia> grid;
        private ListStore<BaseModelIncidencia> ListStoreUnidadGrid = new ListStore<BaseModelIncidencia>();
        ...
        BaseModelIncidencia BMTipoIncidencia = new BaseModelIncidencia();
        
        public WindowGridFiltrarTipoIncidencia(RPCTipoIncidenciaAsync RPCUTipoIncidencia){
            this.RPCTipoIncidencia = RPCUTipoIncidencia;
            window.setHeading("Buscar en Catalogo Tipo de Reporte");
            ...
            grid = new Grid<BaseModelIncidencia>(ListStoreUnidadGrid,ColumnModelGrid);
            ...
            PanelGridFolios.setHeading("Listado de Tipos Reportes");
        ...
        public BaseModelIncidencia getBMSeleccionado(){
            return BMTipoIncidencia;
        }
        public ModeloTipoIncidencia getModeloSeleccionado(){
            return BMTipoIncidencia.getModeloDatos();
        }
        public void show(String CadenaFiltro){
            window.setVisible(true);
            window.mask("Obteniendo Listado de Tipos Reportes");
            window.center();
            window.setFocusWidget(TextFieldBuscar);
            TextFieldBuscar.setValue(CadenaFiltro);
            RPCTipoIncidencia.getListado(CadenaFiltro,RetornoActualizarListStoreGrid);
        }
        public void hide(){
        ...
        class EVENTO_BOTON_ACTUALIZAR extends SelectionListener<ButtonEvent>{
            ...
            public void componentSelected(ButtonEvent ce) {
                ...
                if(CadenaFiltro != null && CadenaFiltro.length() > 0){
                    ...
                    MessageBox.alert("Alerta", "Debe teclar el nombre de alguna Tipo Reporte para realizar la busqueda",null);
                    window.unmask();
                }
            }
        }
        ...
        class EVENTO_SELECCIONAR_GRID implements Listener<SelectionChangedEvent<BaseModelIncidencia>>{
            public void handleEvent(SelectionChangedEvent<BaseModelIncidencia> be){
                BMTipoIncidencia = be.getSelectedItem();
            }
        }
        class RETORNO_ACTUALIZAR_LISTSTORE_GRID implements AsyncCallback<String[][]>{
            public void onSuccess(String[][] result) {
                String obj[][] = (String[][])result;
                ListStoreUnidadGrid = new ListStore<BaseModelIncidencia>();

                int posy = 0;
                while(posy < obj.length){
                    ListStoreUnidadGrid.add(new BaseModelIncidencia(Integer.parseInt(obj[posy][0].toString()),obj[posy][1].toString()));
                    posy++;
                }
